...
// errorService is an AllowListService whose ReadParsed always fails, used to
// exercise the error-handling branches of IsEnabled, CheckURL and DialContext.
type errorService struct{}

func (errorService) ReadParsed(id portainer.AllowListKey) (*portainer.ParsedAllowList, error) {
	return nil, errors.New("read failed")
}

func TestIsEnabled_ReadParsedError(t *testing.T) {
	err := Configure(errorService{})
	require.NoError(t, err)
	t.Cleanup(func() { globalDialer.Store(nil) })
...
func TestCheckURL_NoDialerConfigured(t *testing.T) {
	globalDialer.Store(nil)

	err := CheckURL(t.Context(), "http://example.com")
	require.NoError(t, err)
}

func TestCheckURL_ReadParsedError(t *testing.T) {
	err := Configure(errorService{})
	require.NoError(t, err)
	t.Cleanup(func() { globalDialer.Store(nil) })
...
func TestDialContext_ReadParsedError(t *testing.T) {
	err := Configure(errorService{})
	require.NoError(t, err)
	t.Cleanup(func() { globalDialer.Store(nil) })
...
func TestHTTP1Only(t *testing.T) {
	t.Parallel()

	p := HTTP1Only()
	require.NotNil(t, p)
	require.True(t, p.HTTP1())
}

// stubRoundTripper is an http.RoundTripper that is not an *http.Transport, used
// to exercise WrapDefaultTransport's type-assertion failure branch.
...
func TestWrapDefaultTransport_NotHTTPTransport(t *testing.T) {
	original := http.DefaultTransport
	t.Cleanup(func() { http.DefaultTransport = original })

	http.DefaultTransport = stubRoundTripper{}

	require.False(t, WrapDefaultTransport())
}

func TestWrapDefaultTransport_Success(t *testing.T) {
	original := http.DefaultTransport
	t.Cleanup(func() { http.DefaultTransport = original })
	...
	require.True(t, WrapDefaultTransport())

	wrapped, ok := http.DefaultTransport.(*http.Transport)
	require.True(t, ok)
	require.NotNil(t, wrapped.DialContext)
}
