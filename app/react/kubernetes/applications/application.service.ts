  ...
  try {
    const { data } = await axios.get<T>(
      buildUrl(environmentId, namespace, `${appKind}s`, name),
      {
        ...
        // this logic is to get the latest YAML reponse and not cache the response
        // axios-cache-adapter looks for the response headers to determine if the response should be cached
        // to avoid writing responseInterceptor, we can add a query param to the request url
        params: yaml
          ? {
              _: Date.now(),
            }
