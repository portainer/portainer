...
// Helm's getters install their own transport and bypass http.DefaultTransport, so the
// transport passed here must be created via ssrf.NewTransport.
func helmGetterTransport(m dsl.Matcher) {
	m.Match(`getter.WithTransport(&http.Transport{$*_})`).
		Report(`getter.WithTransport called with a bare *http.Transport; use ssrf.NewTransport(tlsConfig) as Helm getters bypass http.DefaultTransport`)
}
