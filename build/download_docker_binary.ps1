...
Move-Item -Path "$($download_folder)/docker/*" -Destination "dist/"
Remove-Item -Path "dist/cli-plugins" -Recurse -Force -Confirm:$false
Remove-Item -Path "dist/dockerd.exe" -Recurse -Force -Confirm:$false
