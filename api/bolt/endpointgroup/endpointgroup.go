...
// EndpointGroup returns an environment group by ID.
...
// UpdateEndpointGroup updates an environment group.
...
// DeleteEndpointGroup deletes an environment group.
func (service *Service) DeleteEndpointGroup(ID portainer.EndpointGroupID) error {
	identifier := internal.Itob(int(ID))
	return internal.DeleteObject(service.connection, BucketName, identifier)
}

// EndpointGroups return an array containing all the environment groups.
...
// CreateEndpointGroup assign an ID to a new environment group and saves it.
