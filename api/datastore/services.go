...
func (store *Store) initServices() error {
	...
	sceneService, err := scene.NewService(store.connection)
	if err != nil {
		return err
	}
...
func (store *Store) Scene() dataservices.ScenesService {
...
func (store *Store) Export(filename string) (err error) {
	...
	if e, err := store.Scene().Scenes(); err != nil {
		if !store.IsErrObjectNotFound(err) {
			logrus.WithError(err).Errorf("Exporting Scenes")
		}
	} else {
		backup.Scene = e
	}
