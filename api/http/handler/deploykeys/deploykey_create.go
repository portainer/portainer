...
func (payload *deploykeyCreatePayload) Validate(r *http.Request) error {
	if govalidator.IsNull(payload.Name) {
		return portainer.Error("Invalid key name")
	}
	return nil
}
...
func (handler *Handler) DeploykeyCreate(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	for _, deploykey := range deploykeys {
		if deploykey.Name == payload.Name {
			return &httperror.HandlerError{http.StatusConflict, "This name is already associated to a key", portainer.ErrDeploykeyAlreadyExists}
		}
	}
	...
	if err != nil {
		return &httperror.HandlerError{http.StatusInternalServerError, "Unable to persist the key inside the database", err}
	}
