...
type (
	// Manager represents a service used to manage proxies to environments and extensions.
	Manager struct {
...
// CreateAndRegisterEndpointProxy creates a new HTTP reverse proxy based on environment properties and and adds it to the registered proxies.
...
// CreateComposeProxyServer creates a new HTTP reverse proxy based on environment properties and and adds it to the registered proxies.
// It can also be used to create a new HTTP reverse proxy and replace an already registered proxy.
...
// and cleans the k8s environment client cache. DeleteEndpointProxy
