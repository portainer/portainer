    ...
    shell: {
      ...
      run: {
        command: [
          ...
          'docker rm portainer',
          'docker run --privileged -d -p 9000:9000 -v /tmp/portainer:/data -v /var/run/docker.sock:/var/run/docker.sock --name portainer --net host  portainer --no-analytics --templates http://localhost:8080/templates.json --no-auth'
        ].join(';')
      },
      runSwarm: {
        command: [
