    ...
    shell: {
      ...
      run: {
        command: [
          ...
          'docker run -d -p 9000:9000 -v $(pwd)/dist:/app -v /tmp/pdata:/data -v /var/run/docker.sock:/var/run/docker.sock:z --name portainer portainer/base /app/portainer-linux-amd64 --no-analytics -a /app'
        ].join(';')
      }
    },
    replace: {
